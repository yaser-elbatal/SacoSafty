...
function AwarnessEmployee(props) {
    ...
    return (
        ...
        <Subscription subscription={EmployeeNum} variables={{ report_id: props.match.params.id }}>
            {
                ({ loading, error, data }) => {
                    if (loading)
                    ...
                    if (data.training_report_employees.length) {
                        ...
                        return (
                            <Table bordered responsive highlight hover style={{ textAlign: "center", backgroundColor: "white" }}>
                                <thead>
                                    <tr>
                                        <th> # </th>
                                        <th>اسم المتدرب</th>
                                ...
                                <tbody>
                                    {
                                        data.training_report_employees.map((emp, indx) => {
                                            return (
                                                <tr key={indx}>
                                                    <th>{emp.employee}</th>
                                                    <td>{emp.training_employee.employee_user.display_name}</td>
                                                    <td>{emp.signature == null ? "لم يوقع بعض" : emp.training_employee.signature}</td>
                                                    ...
                                                    <td><DeleteEmployee employeeId={emp.employee} /></td>
