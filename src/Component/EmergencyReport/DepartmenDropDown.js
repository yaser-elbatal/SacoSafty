    ...
    render() {
        ...
        let emergancyRep = data.emergency_report.map(reb => {
            ...
            reb.emergency_report_department.map(dep => BranchDep.push({
                dname: dep.emergency_department_technical_department.name_ar,
                did: dep.emergency_department_technical_department.id,
                bname: branch.name_ar, bid: branch.id,
            }))
        });
        ...
        let allDep = Department.map(m => ({ id: m.id, value: m.name_ar }))
        let allBranch = Branches.map(b => ({ id: b.id, value: b.name_ar }))
