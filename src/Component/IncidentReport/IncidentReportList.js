    ...
    render() {
        return (
            <div style={{ marginTop: "50px" }}>
                <Query query={IncidentReport} variables={{
                    filter: this.state.filter ? { branch_id: { "_eq": parseInt(this.state.filter) } } : {}
                }}>
                    {
                        ({ loading, error, data }) => {
                            if (loading)
                            ...
                            if (data.incident_report.length) {
                                ...
                                return (
                                    <Card>
                                        <CardHeader>
                                            <Badge color="danger" style={{ fontSize: "15px", textAlign: "center", padding: "15px" }} >
                                            ...
                                            <Table dark striped bordered style={{ textAlign: "center" }}>
                                                <thead>
                                                    <tr>
                                                        <th> # </th>
                                                        <th>فرع</th>
                                                        <th style={{ marginRight: "20px" }}>تاريخ انشاء التقرير</th>
                                                        <th>وقت الحادثه</th>
                                                        <th>اخر تحديث</th>
                                                        <th>اسم المنشأ</th>
                                                        <th>التفاصيل</th>
                                                        ...
                                                        return (
                                                            <tr key={inc.id}>
                                                                ...
                                                                <td style={{ marginLeft: "30px" }}>
                                                                    <Badge color="info" style={{ padding: "10px", fontSize: "15px", textAlign: "center" }}>
                                                                        {inc.incident_branch.name_ar}
                                                                    </Badge>
                                                                </td>
                                                                ...
                                                                <td>
                                                                    <Link to={`/incident/${inc.id}`} className="btn btn-success">
                                                                        عرض
                                                               </Link>
                                                                </td>
                                                            </tr>
