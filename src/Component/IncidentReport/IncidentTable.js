    ...
    return (
        <Table bordered responsive hover style={{ backgroundColor: "white" }}>
            <tbody>
                ...
                <tr>
                    <td>الفرع</td>
                    <td style={{ fontSize: "20px" }}> <Badge style={{ padding: "10px" }} color="info">{incidentReport.incident_branch.name}</Badge></td>
                </tr>
                <tr>
                    <td>حاله التقرير</td>
                    <td style={{ fontSize: "20px" }}> <Badge style={{ padding: "10px" }} color={incidentReport.status = 0 ? "dark" : "warning"}>{incidentReport.status = 0 ? "قديم" : "جديد"}</Badge></td>
                </tr>
                <tr>
                    <td >الابلاغ عن </td>
                    <td style={{ fontSize: "20px" }}>
                        <Badge color="dark" style={{ padding: "10px" }}>
                            {incidentReport.report_for == 1 ? "اصابه" : incidentReport.report_for == 2 ? "حريق" : "احتمال حدوث اصابه"}
                        </Badge>
                    </td>
                </tr>
                ...
                <tr>
                    <td>هل يوجد اصابه؟ </td>
                    <td><Badge style={{ padding: "10px" }} color={incidentReport.anyone_injured ? "danger" : "warning"}>{incidentReport.anyone_injured ? "نعم يوجد اصابات" : "لا يوجد اصابات"}</Badge></td>
                </tr>
                <tr>
                    <td> ماذا فعلت ؟</td>
                    <td> <Badge style={{ padding: "15px" }} color="warning">
                        {incidentReport.your_action}
                    </Badge></td>
                </tr>
                <tr>
                    <td>هل تم ابلاغ المدير؟</td>
                    <td>{incidentReport.reported_for_manager ?
                        <i style={{ fontSize: "25px", color: "green" }} className="fa fa-check-circle"></i>
                        : <i style={{ fontSize: "25px", color: "red" }} className="fa fa-times-circle"></i>}</td>
                </tr>
                <tr>
                    <td>فرصه تكرار هذا الحادث</td>
                    <td><Badge style={{ padding: "15px" }} color={incidentReport.chance_of_occurence == 1 || incidentReport.chance_of_occurence == 2 ? "danger" : "warning"}>{incidentReport.chance_of_occurence == 1 || incidentReport.chance_of_occurence == 2 ? "حادث عرضي" : incidentReport.chance_of_occurence == 3 ? "اسبوعي" : incidentReport.chance_of_occurence == 4 ? "شهريا" : "كل سته اشهر"}</Badge></td>
                </tr>
                <tr>
                    <td>وصف الحادث</td>
                ...
                <tr>
                    ...
                    <td>
                        <div style={{ display: "flex", flexWrap: "wrap", border: "1px solid #D6D6D6", marginTop: "5px" }}>
                            {
                                incidentReport.employee_signature.split(',').filter(Boolean).map((img, ind) => {
                                    let editedImg = !img.includes("http") ? `https://${img}` : img;
                                    return (
                                        <div key={ind} style={{ width: "40px", margin: "7px 7px 7px 0px" }}>
                                            <ModalImage small={editedImg} large={editedImg} alt={`صورة توضيحية - ${ind + 1}`} />
                                        </div>)
                                })}
                        </div>
                    </td>
                ...
                <tr>
                    ...
                    <td>{incidentReport.engineer_action_taken ?
                        <i style={{ fontSize: "25px", color: "green" }} className="fa fa-check-circle"></i>
                        : <i style={{ fontSize: "25px", color: "red" }} className="fa fa-times-circle"></i>}</td>
                </tr>
                <tr>
                    <td>هل قام المهندس بتبليغ القسم؟</td>
                    <td>{incidentReport.engineer_reported_to_dept ?
                        <i style={{ fontSize: "25px", color: "green" }} className="fa fa-check-circle"></i>
                        : <i style={{ fontSize: "25px", color: "red" }} className="fa fa-times-circle"></i>}</td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <div style={{ display: "flex", flexWrap: "wrap", border: "1px solid #D6D6D6", marginTop: "5px" }}>
                            {
                                incidentReport.engineer_signature && incidentReport.engineer_signature.split(',').filter(Boolean).map((img, ind) => {
                                    let editedImg = !img.includes("http") ? `https://${img}` : img;
                                    return (
                                        <div key={ind} style={{ width: "40px", margin: "7px 7px 7px 0px" }}>
                                            <ModalImage small={editedImg} large={editedImg} alt={`صورة توضيحية - ${ind + 1}`} />
                ...
                <tr>
                    ...
                    <td>{incidentReport.manager_require_revision ?
                        <i style={{ fontSize: "25px", color: "green" }} className="fa fa-check-circle"></i>
                        : <i style={{ fontSize: "25px", color: "red" }} className="fa fa-times-circle"></i>}</td>
                </tr>
            </tbody>
