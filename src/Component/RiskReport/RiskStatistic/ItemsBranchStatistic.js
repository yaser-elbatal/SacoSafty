        ...
        }}>
            {
                  ({ loading, error, data }) => {
                    ...
                    if(data.risk_assessment.length){
                        ...
                        let bids = unique(data.risk_assessment.map(r=>({bid: r.branch_id, created_at: r.created_at, bname: r.risk_assessment_branch.name_ar, reps: []})), 'bid')

                        bids = (!this.state.date)?
                        bids: bids.filter(rep=>this.state.date == (`${rep.created_at.split("-")[0]}-${rep.created_at.split("-")[1]}`))


                        let bid = bids.map(b=> { b.reps = data.risk_assessment.filter(ra=> b.bid == ra.branch_id)
                            return b  })
                        ...
                        return <CategoriesStatistic items={items} RiskItems={bid} />
