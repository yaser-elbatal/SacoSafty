            ...
            <Query query={List_Branches_Companies}>
              {
                ({ loading, error, data }) => {
                  ...
                  if (data.maintainance_company.length) {
                    ...
                    let
                      dropdownBranches = data.branch.map(brnch => (
                        { id: brnch.id, value: `${brnch.name_ar} | ${brnch.name}` })
                      ),
                          ...
                          <Col xl={12}>
                            <div style={{ marginBottom: "15px", display: "inline-flex" }}>
                              <span style={{ marginLeft: "10px" }}>
                                <DrpDwn data={[{ id: 0, value: "كل الفروع" }, ...dropdownBranches]} color="instagram" onChange={this.onChange} />
                              </span>
                              <span style={{ marginLeft: "10px" }}>
