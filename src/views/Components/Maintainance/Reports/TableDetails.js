                    ...
                    this.props.data.map((dat, ind) => {
                        ...
                        return (
                            <Card key={ind} style={{ marginTop: "5px" }}>
                                <CardHeader>
                                    <span><b>{`${ind + 1} - التصنيف [${dat.name_ar}]`}</b></span>
                                    {dat.isNeglected && <span style={{ marginRight: "10px" }}>
                                        <Badge style={{ fontSize: "15px" }} color="danger">تم حذف التصنيف</Badge>
                                    </span>}
                                </CardHeader>
                                <CardBody style={{ backgroundColor: "#e8e8e861" }}>
                                    {
                                        dat.items.map((itm, ind, itms) => (
                                            <div key={ind} style={{ backgroundColor: "#f7e4c8", padding: "10px", marginBottom: "10px" }} >
                                                <div style={{ display: "flex", justifyContent: "space-between", alignContent: "center" }}>
                                                    <div style={{ display: "inline", fontWeight: "600", display: "flex", justifyContent: "flex-start" }}>
                                                        <div style={{ marginLeft: "10px" }}>{`${ind + 1} - العنصر [${itm.name_ar}]`}</div>
                                                ...
                                                <div style={{fontWeight: 500, padding: "15px"}}>عدد الأجزاء التى تم تغييرها ({itm.details.amount})</div>
                                            </div>
                                        )
                                        )
                                    }
