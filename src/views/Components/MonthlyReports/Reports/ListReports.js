                ...
                ({ loading, error, data }) => {
                  ...
                  if (data.branches.length) {
                    ...
                    let dropdownBranches = data.branches.map(brnch => (
                      { id: brnch.monthely_branch.id, value: `${brnch.monthely_branch.name_ar} | ${brnch.monthely_branch.name}` })
                    ),
                      branchId = this.state.selectedBranchId == 0 ? false : this.state.selectedBranchId;
                    ...
                    let dropdowBStatus = [
                      ...
                      selectedStatus = this.state.selectedStatus == 0 ? false : this.state.selectedStatus;

                    let dropdownDates = data.locations
                      .filter((elm, i, arr) => arr.findIndex(f => f.month == elm.month && f.year == elm.year) === i)
                      .map((dt, ind) => (
                        { id: (ind + 1), value: `${dt.month}/${dt.year}` }
                      )),
                      selectedDate = this.state.selectedDate == 0 || !this.state.selectedDate.split('').includes('/') ?
                        false : this.state.selectedDate;
                    ...
                    return (
                      <div>
                          ...
                          <Col xl={12}>
                            <div style={{ marginBottom: "15px", display: "inline-flex" }}>
                              <span style={{ marginLeft: "10px" }}>
                                <DrpDwn data={[{ id: 0, value: "كل الفروع" }, ...dropdownBranches]} color="instagram" onChange={this.onChange1} />
                              </span>
                              <span style={{ marginLeft: "10px" }}>
                                <DrpDwn data={[{ id: 0, value: "كل الحالات" }, ...dropdowBStatus]} color="instagram" onChange={this.onChange2} />
                              </span>
                              <span>
                        ...
                        <Row>
                          <Col xl={12}>
                            <ReportsTable branchId={branchId} status={selectedStatus} date={selectedDate} />
                          </Col>
                        </Row>
                      </div>
                    )
