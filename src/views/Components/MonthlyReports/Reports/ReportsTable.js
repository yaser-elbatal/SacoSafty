...
export default class ListReports extends Component {
    ...
    componentDidMount() {
        this.setState({
            branchId: this.props.branchId, targetCursor: this.props.targetCursor || "",
            status: this.props.status, targetCursor: this.props.targetCursor || "",
            date: this.props.date, targetCursor: this.props.targetCursor || "",
        })
    ...
    componentDidUpdate() {
        ...
        if (branchId != this.state.branchId || status != this.state.status || date != this.state.date)
    ...
    ReportRow({ rep }) {
        let ind = rep.index + 1,
            ...
            status = rep.status == 0 ? { val: "جديد", clr: "primary" } :
                rep.status == 1 ? { val: "تحت المراجعة", clr: "warning" } :
        ...
        return (
            <tr>
                ...
                {!rep.branchId && <td><Link to={repLink}>{rep.monthely_branch.name}</Link></td>}
                <td>{`${rep.month} / ${rep.year}`}</td>
                <td><Link to={createdByLink}>{rep.monthly_report_created_by.username}</Link></td>
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (
            nextState.targetCursor != this.state.targetCursor
        ) return false
        return true;
    }
    ...
    ftchMor = fetchMore => {
        fetchMore({
            variables: {
                ...
                filterBranch: {
                    ...(this.state.branchId && { bracnh_id: { "_eq": parseInt(this.state.branchId) } }),
                    ...(this.state.status && { status: { "_eq": parseInt(this.state.status) - 1 } }),
                    ...(this.state.date && {
                        month: { "_eq": this.state.date.split('/')[0] }, year: { "_eq": this.state.date.split('/')[1] }
                    }),
            ...
            updateQuery: (prev, { fetchMoreResult }) => {
                this.setState({
                    targetCursor: !fetchMoreResult.monthely_report.length ?
                        -1 : fetchMoreResult.monthely_report[fetchMoreResult.monthely_report.length - 1].id
                });
                if (!fetchMoreResult) return prev;
                return Object.assign({}, prev, {
                    monthely_report: [...prev.monthely_report, ...fetchMoreResult.monthely_report]
                });
            }
        });
    }
        ...
        return (
            <div className="animated fadeIn">
                <Query
                    ...
                    variables={{
                        limit: this.state.limit,
                        filterBranch: !this.state.branchId && !this.state.status && !this.state.date ? {} :
                            {
                                ...(this.state.branchId && { "bracnh_id": { "_eq": parseInt(this.state.branchId) }, }),
                                ...(this.state.status && { "status": { "_eq": parseInt(this.state.status) - 1 }, }),
                                ...(this.state.date && {
                        ...
                        ({ loading, error, data, fetchMore }) => {
                            ...
                            if (data.monthely_report.length) {
                                ...
                                return (
                                    <div>
                                        <Card>
                                            <CardHeader><b>{this.props.branchId ? "تقارير الفرع" : "آخر التقارير"}</b></CardHeader>
                                            <CardBody>
                                                    ...
                                                    <tbody>
                                                        {
                                                            data.monthely_report.map((rep, index) =>
                                                                <this.ReportRow key={index} rep={{ ...rep, index, branchId: this.props.branchId }} />
                                                            )
                                                        }
                                                    </tbody>
                                                </Table>
