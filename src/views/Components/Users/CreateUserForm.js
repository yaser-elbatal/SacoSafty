...
export default class CreateCatForm extends Component {
    ...
    componentDidMount() {
        if (this.props.updateData && this.props.dropdownData)
            this.props.updateData({ dropdown: { [this.props.dropdownData.fieldName]: parseInt(this.props.dropdownData.data[0].id) } })
    ...
    selectHandle = (e) => {
        if (this.props.updateData)
            this.props.updateData({ dropdown: { [this.props.dropdownData.fieldName]: parseInt(e.id) } })
    }
        ...
        return (
            <Form>
                {this.props.dropdownData &&
                    <FormGroup>
                        <DrpDwn data={this.props.dropdownData.data} color="primary" onChange={this.selectHandle} />
                    </FormGroup>
                }
                <FormGroup>
                    <Label for="username">اسم الدخول</Label>
                    <Input type="text" name="username" onChange={this.changeHandle} id="username" placeholder="اسم الدخول" />
                </FormGroup>
                <FormGroup>
                ...
                {this.props.dropdownData && this.props.dropdownData.fieldName == "branch_id" &&
                    <Fragment>
                        <FormGroup>
                            <Label for="email">البريد الإلكترونى</Label>
                            <Input type="email" name="email" onChange={this.changeHandle} id="email" placeholder="البريد الإلكترونى" />
                        </FormGroup>
                        <FormGroup>
                            <Label for="employee_number">رقم الموظف</Label>
                            <Input type="number" min="1" name="employee_number" onChange={this.changeHandle} id="employee_number" placeholder="رقم الموظف" />
                        </FormGroup>
                    </Fragment>
                }
                <FormGroup>
